...
import {
  PROXY_SERVICES as APPLICATION_CONFIGURATION_PROXY_SERVICES,
  ApplicationConfigurationManager,
  EleoncoreApplicationConfigurationDto,
} from '@eleon/app-config.lib';
import {
  PROXY_SERVICES as SYSTEM_PROXY_SERVICES,
  SystemLogService,
} from '@eleon/system-services.lib';
import { PROXY_SERVICES as IDENTITY_QUERYING_PROXY_SERVICES } from '@eleon/identity-querying.lib';
import {
  PermissionService,
  QuickReloginService,
} from '@eleon/typescript-sdk.lib';
import {
  LocalizationService,
...
import {
  ...
  NgModuleLoaderManager,
  NoopConfigStateService,
  NoopSessionStateService,
  NoopAuthService,
...
} from '@eleon/contracts.lib';
import {
  ClientLogService,
  sendSystemLogs,
  EleoncoreErrorHandlingService,
} from '@eleon/logging.lib';
import {
  EleoncoreError,
  IAppearanceService,
  IVPortalMenuService,
  IVPortalTopbarService,
} from '@eleon/contracts.lib';
...
import {
  ...
  IDynamicLocalizationService,
  IModuleSettingService,
  IQuickReloginService,
  ...
  IFileArchiveSelectionDialogService,
...
import {
  PROXY_SERVICES as STORAGE_PROXY_SERVICES,
  LightweightStorageItemService,
} from '@eleon/storage.lib';
import { CachedLightweightStorageService } from '@eleon/storage.lib';

export function registerBasicProviders(
  appConfiguration?: IApplicationConfigurationManager,
  moduleLoader?: IModuleLoaderManager,
  appId?: string,
  defaultAppConfig?: EleoncoreApplicationConfigurationDto
) {
  return [
    ...
    },
    {
      ...
      useFactory: (
        ...
        return new NgWebPushCommunicationService(
          swPush,
          authManager,
          appConfig
        );
    ...
    },
    {
      ...
      useFactory: (
        configState: IApplicationConfigurationManager,
        sessionState: ISessionStateService
      ) => {
    ...
    },
    {
      ...
      useFactory: (
        ...
        if (isPlatformBrowser(platformId)) {
          const clientAuthManager = new ClientAuthManager(
    ...
    },
    {
      ...
      useClass: IApiKeySelectionDialogService,
    },
    {
      provide: ITenantDomainsSelectionDialogService,
      useClass: ITenantDomainsSelectionDialogService,
    },
    {
    ...
    },
    {
      ...
      useFactory: (
        ...
        sysLogService: SystemLogService,
        authService: IAuthManager
      ) => {
        return () =>
          sendSystemLogs(
...
function registerServiceWorker() {
  if (
    ...
    !window.applicationConfiguration?.getApplicationSettingBoolean(
      'UseServiceWorker'
    )
  ) {
    return [];
...
function registerAppConfiguration(
  appConfiguration?: IApplicationConfigurationManager,
  appId?: string,
  defaultAppConfig?: EleoncoreApplicationConfigurationDto
): Provider[] {
  ...
  console.log(
    'Application configuration is not provided. Creating new instance.'
  );
