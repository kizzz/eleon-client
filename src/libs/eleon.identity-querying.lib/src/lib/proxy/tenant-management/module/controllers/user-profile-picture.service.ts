...
import type { SetUserProfilePictureRequest } from '../users/models';
...
export class UserProfilePictureService {
  ...
  setUserProfilePictureByRequest(request: SetUserProfilePictureRequest, config?: Partial<any>): Observable<boolean> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonauth'] || '';
    const baseUrl = apiBase + '/api/CoreInfrastructure/UserProfilePicture/SetUserProfilePicture';
