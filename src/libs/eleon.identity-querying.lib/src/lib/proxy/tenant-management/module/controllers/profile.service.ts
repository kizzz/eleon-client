...
export class ProfileService {
  ...
  changePassword(input: ChangePasswordInput, config?: Partial<any>): Observable<void> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonauth'] || '';
    const baseUrl = apiBase + '/api/identity-querying/profile/ChangePasswordAsync';
  ...
  get(config?: Partial<any>): Observable<ProfileDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonauth'] || '';
    const baseUrl = apiBase + '/api/identity-querying/profile/GetAsync';
      ...
      this.authFetch(eleoncoreApiUrl, options)
        .then(res => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as ProfileDto);
							subscriber.complete();
						});
					} else {
						return res.text().then(data => {
  ...
  update(input: UpdateProfileDto, config?: Partial<any>): Observable<ProfileDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonauth'] || '';
    const baseUrl = apiBase + '/api/identity-querying/profile/UpdateAsync';
