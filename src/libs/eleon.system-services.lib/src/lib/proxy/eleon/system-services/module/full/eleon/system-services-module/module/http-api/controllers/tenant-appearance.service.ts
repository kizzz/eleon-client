...
import type { TenantAppearanceSettingDto } from '../../application/contracts/tenant-appearance/models';

import { Observable } from 'rxjs/internal/Observable';


export class TenantAppearanceService {
  ...
  getTenantAppearanceSettings(config?: Partial<any>): Observable<TenantAppearanceSettingDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonauth'] || '';
    const baseUrl = apiBase + '/api/system-services/appearance';
    ...
    return new Observable<TenantAppearanceSettingDto>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as TenantAppearanceSettingDto);
							subscriber.complete();
						});
					} else {
