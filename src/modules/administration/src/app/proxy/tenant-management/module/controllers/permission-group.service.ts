...
import type { PermissionGroupCategory } from '../permission-groups/models';
...
export class PermissionGroupService {
  ...
  getPermissionGroups(config?: Partial<any>): Observable<PermissionGroupCategory[]> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/TenantManagement/PermissionGroups/GetPermissionGroups';
    ...
    return new Observable<PermissionGroupCategory[]>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as PermissionGroupCategory[]);
							subscriber.complete();
						});
					} else {
