...
export class TimeZoneSettingsService {
  ...
  get(config?: Partial<any>): Observable<string> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/TenantManagement/TimeZoneSettings/GetAsync';
  ...
  getTimezones(config?: Partial<any>): Observable<NameValue[]> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/TenantManagement/TimeZoneSettings/GetTimezonesAsync';
    ...
    return new Observable<NameValue[]>(subscriber => {
      this.authFetch(eleoncoreApiUrl, options)
        .then(res => {
          if (!res.ok) {
            if (!config?.skipHandleError) {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as NameValue[]);
							subscriber.complete();
						});
					} else {
						return res.text().then(data => {
  ...
  update(timezone: string, config?: Partial<any>): Observable<void> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/TenantManagement/TimeZoneSettings/GetTimezonesAsync';
