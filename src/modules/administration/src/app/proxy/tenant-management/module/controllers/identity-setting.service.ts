...
import type { IdentitySettingDto, SetIdentitySettingsRequest } from '../settings/models';
...
export class IdentitySettingService {
  ...
  getIdentitySettings(config?: Partial<any>): Observable<IdentitySettingDto[]> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Infrastructure/IdentitySettings/GetIdentitySettings';
    ...
    return new Observable<IdentitySettingDto[]>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as IdentitySettingDto[]);
							subscriber.complete();
						});
					} else {
  ...
  setIdentitySettingsByRequest(request: SetIdentitySettingsRequest, config?: Partial<any>): Observable<boolean> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Infrastructure/IdentitySettings/SetIdentitySettings';
