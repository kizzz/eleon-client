...
@Component({
    ...
    template: `
		<ng-container>
            <div *ngIf="root && item.visible !== false" class="layout-menuitem-root-text py-0">{{item.label | abpLocalization}}</div>
			<a *ngIf="(!item.routerLink || item.items?.length) && item.visible !== false" [attr.href]="item.url" (click)="itemClick($event)"
			...
			<a *ngIf="(item.routerLink && !item.items?.length) && item.visible !== false" (click)="itemClick($event)" [ngClass]="item.class" 
