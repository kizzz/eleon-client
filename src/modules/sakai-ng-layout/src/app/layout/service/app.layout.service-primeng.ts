...
import { ILayoutService, LayoutState, LayoutInitConfig } from '@eleon/angular-sdk.lib';
@Injectable({
  providedIn: "root",
})
export class LayoutService extends ILayoutService  {
  ...
  constructor(
    ...
    super();
    this.assetLoader.addModuleHeadAssetWithId('Primeng Layout_Module', 'assets/layout/styles/theme/lara-light-indigo/theme.css', 'theme-css');
    this.assetLoader.addHeadCss('https://fonts.googleapis.com/css?family=Material+Icons|Material+Icons+Outlined|Material+Icons+Two+Tone|Material+Icons+Round|Material+Icons+Sharp|Material+Symbols+Outlined');
    this.assetLoader.addHeadCss('https://fonts.googleapis.com/css2?family=Inter:wght@100..900&display=swap');
    ...
    this.getConfig().subscribe((result: LayoutState) => {
      ...
      // save settings only if "UserSetting" is enabled in the environment
  ...
  getDefaultState(): LayoutState {
    const baseConfig = JSON.parse(
       this.configStateService.getAppConfig()['clientApplication']['properties']
        .find(t => t.key == "LayoutInitConfig")?.value ?? '{}'
    );
    return {
  ...
  updateStyle(state: LayoutState) {
    return (
      state.componentTheme !== this.getDefaultState().componentTheme ||
      state.colorScheme !== this.getDefaultState().colorScheme
    );
  }
  ...
  changeTheme() {
    const { colorScheme, theme } = this.config();
    const themeLink = <HTMLLinkElement>document.getElementById('theme-css');
    const themeLinkHref = themeLink.getAttribute('href')!;
    const newHref = themeLinkHref
      .split('/')
      .map((el) =>
        el === this.getDefaultState().theme ? theme : el === `theme-${this.getDefaultState().colorScheme}` ? `theme-${colorScheme}` : el
      )
      .join('/');
