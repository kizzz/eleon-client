...
export class ExampleService {
  ...
  get(config?: Partial<any>): Observable<SampleDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/module/example';
    ...
    return new Observable<SampleDto>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as SampleDto);
							subscriber.complete();
						});
					} else {
  ...
  getAuthorized(config?: Partial<any>): Observable<SampleDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/module/example/authorized';
