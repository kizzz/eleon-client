...
export class TemplateService {
  ...
  applyTemplate(input: ApplyTemplateInput, cancellationToken?: any, config?: Partial<any>): Observable<string> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/templating/templates/Apply';
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as TemplateDto);
							subscriber.complete();
						});
					} else {
						return res.text().then(data => {
  ...
  delete(id: string, cancellationToken?: any, config?: Partial<any>): Observable<void> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/templating/templates/Delete';
  ...
  get(id: string, cancellationToken?: any, config?: Partial<any>): Observable<TemplateDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/templating/templates/GetById';
  ...
  getByNameAndTypeByNameAndTypeAndCancellationToken(name: string, type: TemplateType, cancellationToken?: any, config?: Partial<any>): Observable<TemplateDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/templating/templates/GetByNameAndType';
  ...
  getList(input: GetTemplateListInput, cancellationToken?: any, config?: Partial<any>): Observable<PagedResultDto<MinimalTemplateDto>> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/templating/templates/GetList';
    ...
    const queryString = (() => {
...

      {
        ...
        if (
          ...
          qp.append('format', String(raw));
        }
      }

      {
        const raw = input.isSystem;
        if (
          raw !== undefined &&
          raw !== null &&
          (typeof raw !== 'string' || raw !== '') &&
    ...
    return new Observable<PagedResultDto<MinimalTemplateDto>>(subscriber => {
      this.authFetch(eleoncoreApiUrl, options)
        .then(res => {
          if (!res.ok) {
            if (!config?.skipHandleError) {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as PagedResultDto<MinimalTemplateDto>);
							subscriber.complete();
						});
					} else {
						return res.text().then(data => {
