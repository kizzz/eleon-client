...
import { CommonTenantDto, ITenantService, PipesModule, RequiredMarkModule, SharedModule } from '@eleon/angular-sdk.lib'
...
import { ResponsiveTableModule, SharedTableModule, TableCellsModule, TableRowEditorDirective } from "@eleon/primeng-ui.lib"
...
export class MultiTenancyManagementComponent implements OnInit {
  ...
  public update() : Observable<any> {
    if (this.rows.filter(r => r.isEditing === true).length > 0){
      this.messageService.error('TenantManagement::ConnectionString:Update:Failed:FinishEditing');
      return of({});
    }
  ...
  onFinishUpdate(count: number, success: number, fail: number){
    if (success + fail >= count){
      this.loading = false;
      if (fail > 0){
        this.messageService.error('TenantManagement::ConnectionStrings:Update:Failed', fail.toString(), count.toString());
      }
      else {
        this.messageService.success('TenantManagement::ConnectionStrings:Update:Success');
      }
    }
  }
  ...
  getTenantName(tenantId?: string){
    const res = !tenantId ? this.commonTenants.filter(t => !t.id) : this.commonTenants.filter(t => t.id === tenantId);
  ...
  tenantConString: { tenantId: string, connectionString: string, status: string }[] = []
  tenants: CommonTenantDto[] = [];
    ...
    this.rows = [];
		...
		.pipe(
			finalize(() => this.loading = false),
			catchError((error) => {
				this.messageService.error('TenantManagement::ConnectionStrings:Load:Failed')
				return throwError(() => error);
			})
		)
		.subscribe(res => {
      ...
      this.commonTenants.forEach(t => {
        this.loading = true;
        this.applicationConnectionStringService.get(t.id, this.applicationName)
          .pipe(
            finalize(() => this.loading = false),
            catchError((error) => {
              this.messageService.error('TenantManagement::ConnectionStrings:Load:Failed')
          ...
          .subscribe((conString) =>{
            ...
            }].sort((r1, r2) => r2.data.tenantId.localeCompare(r1.data.tenantId));
          })
      })
		});
	}
  ...
  useDedicated(useDedicatedDb: boolean){
    ...
    this.clientAppsService
      ...
      .pipe(
        finalize(() => this.loading = false),
        catchError((error) => {
          this.messageService.error('TenantManagement::ConnectionString:UseDedicated:Failed');
          return throwError(() => error);
        })
      )
      .subscribe(_ => {
        this.messageService.success('TenantManagement::ConnectionString:UseDedicated:Success')
      });
  }
}
