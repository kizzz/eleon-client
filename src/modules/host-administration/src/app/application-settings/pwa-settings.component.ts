...
import { getDefaultPwaSetting, validatePwaSettingJson } from './pwa-settings.helper';
...
@Component({
    ...
    <div class="grid align-items-center">
      <div class="col-2" appRequired>{{ 'TenantManagement::IsPWA' | abpLocalization }}</div>
      <div class="col-10">
        <p-checkbox
          [binary]="true"
...
export class PwaSettingsComponent {
    ...
    ngOnInit(): void {
        ...
        if (!this.pwaSetting?.length) {
            this.pwaSetting = getDefaultPwaSetting(this.node.data.name, '/apps/' + (this.node.data.path ?? ''));
