...
export function validatePwaSettingJson(jsonStr: string): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (!allowedDisplays.includes(setting.display)) {
    errors.push(
      `Invalid 'display', expected one of: ${allowedDisplays.join(', ')}`
    );
  }
  ...
  if (!Array.isArray(setting.icons) || setting.icons.length === 0) {
    ...
    setting.icons.forEach((icon: any, index: number) => {
      if (typeof icon.src !== 'string')
        errors.push(`Icon ${index}: invalid 'src'`);
      if (!isValidSize(icon.sizes))
        errors.push(`Icon ${index}: invalid 'sizes'`);
      if (typeof icon.type !== 'string')
        errors.push(`Icon ${index}: invalid 'type'`);
      if (typeof icon.purpose !== 'string')
        errors.push(`Icon ${index}: invalid 'purpose'`);
    });
  }
