...
import type { StateAuditDto } from '../../dto/audits/models';
...
export class StateAuditService {
  ...
  addByStateAudit(stateAudit: StateAuditDto, config?: Partial<any>): Observable<boolean> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Lifecycle/Audits/State/Add';
    ...
    const options: RequestInit = {
      ...
      body: JSON.stringify(stateAudit),
  ...
  removeById(id: string, config?: Partial<any>): Observable<boolean> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Lifecycle/Audits/State/Remove';
