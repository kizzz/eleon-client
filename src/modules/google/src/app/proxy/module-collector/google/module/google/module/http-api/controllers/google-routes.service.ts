...
import type { OptimizeToursRequestDto, OptimizedToursDto } from '../../application/contracts/route/models';
...
export class GoogleRoutesService {
  ...
  optimizeRoute(model: OptimizeToursRequestDto, config?: Partial<any>): Observable<OptimizedToursDto> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Google/Geocoding/OptimizeRoute';
    ...
    return new Observable<OptimizedToursDto>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as OptimizedToursDto);
							subscriber.complete();
						});
					} else {
