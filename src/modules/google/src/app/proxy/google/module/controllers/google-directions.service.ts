...
import type { DirectionsPath, LatLng } from '../models/models';
...
export class GoogleDirectionsService {
  ...
  getDirectionsByWaypoints(waypoints: LatLng[], config?: Partial<any>): Observable<DirectionsPath> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleonsoft'] || '';
    const baseUrl = apiBase + '/api/Google/Direction/GetDirections';
    ...
    return new Observable<DirectionsPath>(subscriber => {
          ...
          const contentType = res.headers.get("Content-Type") || "";
					if (contentType.includes("application/json")) {
						return res.json().then(data => {
							subscriber.next(data as DirectionsPath);
							subscriber.complete();
						});
					} else {
