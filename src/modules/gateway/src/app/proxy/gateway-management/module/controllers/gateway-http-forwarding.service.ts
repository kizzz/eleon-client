...
import type { GatewayForwardedResponseDto } from '../proxies/models';

import { Observable } from 'rxjs/internal/Observable';


export class GatewayHttpForwardingService {
  ...
  getForwardedRequestByRequestId(requestId: string, config?: Partial<any>): Observable<Blob> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleoncore'] || '';
    const baseUrl = apiBase + '/api/GatewayManagement/GatewayHttpForwarding/GetForwardedRequest';
    ...
    const queryString = (() => {
...

      {
        ...
        if (
          ...
          qp.append('requestId', String(raw));
        }
      }
  ...
  sendForwardedResponseByResponse(response: GatewayForwardedResponseDto, config?: Partial<any>): Observable<boolean> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleoncore'] || '';
    const baseUrl = apiBase + '/api/GatewayManagement/GatewayHttpForwarding/SendForwardedResponse';
    ...
    const queryString = (() => {
...

      {
        ...
        if (
          ...
          qp.append('responseId', String(raw));
        }
      }
