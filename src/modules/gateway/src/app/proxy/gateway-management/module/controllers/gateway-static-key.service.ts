...
export class GatewayStaticKeyService {
  ...
  getStaticKey(config?: Partial<any>): Observable<string> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleoncore'] || '';
    const baseUrl = apiBase + '/api/GatewayManagement/GatewayStaticKey/GetStaticKey';
  ...
  setStaticKeyEnabledByShouldBeEnabled(shouldBeEnabled: boolean, config?: Partial<any>): Observable<void> {
    // baseUrl is already a quoted literal
		const apiBase = window?.['apiBase']?.['eleoncore'] || '';
    const baseUrl = apiBase + '/api/GatewayManagement/GatewayStaticKey/SetStaticKeyEnabled';
    ...
    const queryString = (() => {
...

      {
        ...
        if (
          ...
          qp.append('shouldBeEnabled', String(raw));
        }
      }
